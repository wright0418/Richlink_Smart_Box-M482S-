...
- 第三批進行中：`power_mgmt.*` 已搬移到 `board/` 並完成 build/test 驗證；`gpio.*` 尚待評估搬移時機。
...
- 建議實際搬移順序（由低風險到高風險）：
  ...
  3. `board/`：之後處理 `gpio.*`、`power_mgmt.*` 這類板級相依較高的模組（`power_mgmt.*` 已完成）。
...
- `drivers` 第二批已完成（`timer.*`、`adc.*`、`i2c.*`）；`board` 層目前已完成 `power_mgmt.*`，建議下一步聚焦 `gpio.*` 是否搬移或先保持原位。
